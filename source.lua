...
function lib.CreateLib(kavName, themeList)
    ...
    lib:DraggingEnabled(MainHeader, Main)
    ...
    function lib:ChangeColor(prope,color)
    ...
    function Tabs:NewTab(tabName)
        ...
        return Sections
        -- Добавление управления клавишей Insert
    local function setupInsertToggle()
        local gui = ScreenGui
        local isOpen = true

        input.InputBegan:Connect(function(inputObj)
            if inputObj.KeyCode == Enum.KeyCode.Insert then
                isOpen = not isOpen
                gui.Enabled = isOpen
                if not isOpen then
